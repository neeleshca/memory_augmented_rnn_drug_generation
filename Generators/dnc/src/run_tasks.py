...
num_memory_locations = 200
memory_size = num_memory_locations
word_size = 1024
num_read_heads = 1
num_write_heads = 1
conv_shift_range = 1
...
# gen_data_path = '../data/chembl_small1.smi'
...
class BuildModel(object):
    ...
    def _build_model(self):
        # cell = NTMCell(num_layers, num_units, num_memory_locations, memory_size,
        #     num_read_heads, num_write_heads, addressing_mode='content_and_location',
        #     shift_range=conv_shift_range, reuse=False, output_dim=num_bits_per_vector,
        #     clip_value=clip_value, init_mode=init_mode)
        dnc_core = dnc.DNC(access_config, controller_config, num_bits_per_vector, clip_value)
        ...
        # Batch size
        initial_state_placeholder = dnc_core.initial_state(batch_size, tf.float32)
...
def evaluate(data, start_char = '<', end_char = '>', predict_length = 100):
    ...
    for i in range(2, predict_length):
        ...
        feed_dict={
            inputs_placeholder: full_inputs,
            initial_state_placeholder.access_output: next_state.access_output,
            initial_state_placeholder.access_state: next_state.access_state,
            initial_state_placeholder.controller_state: next_state.controller_state
        })
        for j in range(batch_size):
...
train = 0
generate = 1
num_evaluate = 10 # Call evaluate after every num_evaluate iterations
...
start_time = time.time()
if(generate == 1):
    generate_strings(f'{PATH_TO_GENERATED_FILE}/generated_file_min.txt', num_strings)
