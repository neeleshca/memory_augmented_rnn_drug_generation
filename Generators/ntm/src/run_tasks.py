...
from ntm import NTMControllerState
import argparse
...
def evaluate(data, start_char = '<', end_char = '>', predict_length = 100):
    ...
    for i in range(2, predict_length):
        ...
        feed_dict={
            ...
            initial_state_placeholder.read_vector_list[0]: next_state.read_vector_list[0],
            initial_state_placeholder.w_list[0]: next_state.w_list[0],
            initial_state_placeholder.w_list[1]: next_state.w_list[1],
            initial_state_placeholder.M: next_state.M
        })
        for j in range(batch_size):
...
start_time = time.time()
if(generate == 1):
    generate_strings(f'{PATH_TO_GENERATED_FILE}/ntm_min_1.txt', num_strings)
