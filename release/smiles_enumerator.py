...
class SmilesEnumerator(object):
    ...
    def randomize_smiles(self, smiles):
        ...
        m = Chem.MolFromSmiles(smiles)          #Gives the molecule from the smiles read from input data
        ans = list(range(m.GetNumAtoms()))  
        np.random.shuffle(ans)          #gets random new order of same molecule for non-canonical augmentation
        nm = Chem.RenumberAtoms(m, ans)     #actually shuffles the molecue
        return Chem.MolToSmiles(nm, canonical=self.canonical, isomericSmiles=self.isomericSmiles)       #converts to smiles and returns
